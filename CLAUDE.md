...
- **Some attributes arrive as properties.** Svelte compiles `value`,
  `checked`, `open` and every name in its `DOM_BOOLEAN_ATTRIBUTES` list to
  `element.open = v`, never `setAttribute`. `nodes.ts` defines a forwarding
  accessor for each (`PROPERTY_ATTRIBUTES`); a widget attribute that never
  arrives is the symptom of a name missing from that table.
