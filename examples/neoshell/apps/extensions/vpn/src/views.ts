...
// vpn views: the top bar indicator, registered as vpn.indicator. An ordinary
// bar child — the bar reserves no space for it, so a machine with no VPN
// profile draws nothing at all. The compiled stylesheet is injected once per
// webview document.
...
const plugin: Plugin.Object = {
  ...
  apply(context) {
    ...
    context.effect(() => ui.register('vpn.indicator', svelteFactory(Vpn, { bus })))
  },
}
...
function injectStylesheet(): () => void {
  ...
  link.rel = 'stylesheet'
  link.href = `/plugins/vpn/views.css${versionQueryOf(import.meta.url)}`
