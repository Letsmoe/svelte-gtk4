import { getJson } from '../gjs/http.js'
...
//
// GJS has no `fetch`, so the request goes through the Soup wrapper in
// `gjs/http.ts`; it rejects on a non-2xx status rather than returning a
// response object to check, which is why `fetchJson` only has to catch.
